...
describe('AppController (e2e)', () => {
  ...
  describe("Update User", () => {
    ...
    it('Return erro when user id not find', async () => {
      const {body} = await request(app.getHttpServer()).post(`/users/update/error}`)
      .send({
        name: 'updated',
        email: 'updated@gmail.com',
        password: 'updated',
      ...
      expect(body).toHaveProperty("message", "User not find")
